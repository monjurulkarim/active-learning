...
def pseudo_color(class_id, masked_image,mask):
    masked_image = apply_mask(masked_image, mask,[1.0, 0.0, 0.0],alpha=0.5)
    return
...
def display_instances(f_ref,image,file,filename,filesize,json1, boxes, masks, class_ids, class_names, pseudo_a,pseudo_b,pseudo_c,pseudo_d,pseudo_e, a,b,c,d,e,cl,Xco,Yco,Xcor1,Ycor1,Xcor2,Ycor2,Xcor3,Ycor3,Xcor4,Ycor4,Xcor5,Ycor5,
                      scores=None, title="",
    ...
    pseudo_a_vertices =[]
    # for bb in b:
    #     pseudo_mask = pseudo_b[:, : , bb]
    #     padded_pseudo_mask = np.zeros((pseudo_mask.shape[0] + 2, pseudo_mask.shape[1] +2), dtype=np.uint8)
    #     padded_pseudo_mask[1:-1, 1:-1] = pseudo_mask
    #     pseudo_countours = find_contours(padded_pseudo_mask, 0.5)
    #     for pseudo_items in pseudo_countours: pseudo_b_vertices.append(pseudo_items)
    # for aa in a:
    #     pseudo_mask = pseudo_a[:, : , aa]
    #     padded_pseudo_mask = np.zeros((pseudo_mask.shape[0] + 2, pseudo_mask.shape[1] +2), dtype=np.uint8)
    #     padded_pseudo_mask[1:-1, 1:-1] = pseudo_mask
    #     pseudo_countours = find_contours(padded_pseudo_mask, 0.5)
    #     for pseudo_items in pseudo_countours: pseudo_a_vertices.append(pseudo_items)
    ...
    for x,y in enumerate(scores):
        ...
        if y < 0.95:
            ...
            # print('vertices_low :', vertices_low)
            ...
            if class_low == 7: #if the low scoring object is pier
                ...
                for aa,bb in enumerate (class_ids):
                    ...
                    if bb == 5: # locate the pier cap
                        ...
                        # print('vertices_related :', vertices_related)
    ...
    if f_ref == 'f1':
        ...
        print('scores_class_ids : ', scores)
        for xx,yy in enumerate(e):
            if yy == 7:
                min_dist = []
                for mm,nn in enumerate(class_ids):
                    if nn == 7:
                        distance = calculateDistance(Xcor5[xx], Ycor5[xx], boxes[mm][1], boxes[mm][0])
                        min_dist.append(distance)
                min_dist.sort()
                pseudo_mask = pseudo_e[:,:,xx]
                print('minimum_distance for :', xx, 'is : ', min_dist)
                if min_dist[0] > 50:
                    for jj, kk in enumerate(d):
                        if kk == 7:
                            min_dist1 = []
                            for jjj,kkk in enumerate(class_ids):
                                if kkk == 7:
                                    distance = calculateDistance(Xcor4[jj], Ycor4[jj], boxes[jjj][1], boxes[jjj][0])
                                    min_dist1.append(distance)
                            min_dist1.sort()
                            print(min_dist1)
                            if min_dist1[0] > 50:
                                if Ycor4[jj] > 300:
                                    print(' recovered with pseudo mask')
                                    pseudo_color(class_id, masked_image, pseudo_mask)
                                    break
                                else:
                                    continue
    elif f_ref == 'f2':
        ...
        print('class_ids :', class_ids)
        print('scores_class_ids : ', scores)
        for xx,yy in enumerate(a):
            if yy == 7:
                min_dist = []
                for mm,nn in enumerate(class_ids):
                    if nn == 7:
                        distance = calculateDistance(Xcor1[xx], Ycor1[xx], boxes[mm][1], boxes[mm][0])
                        min_dist.append(distance)
                min_dist.sort()
                pseudo_mask = pseudo_a[:,:,xx]
                print('minimum_distance for :', xx, 'is : ', min_dist)
                if min_dist[0] > 50:
                    for jj, kk in enumerate(e):
                        if kk == 7:
                            min_dist1 = []
                            for jjj,kkk in enumerate(class_ids):
                                if kkk == 7:
                                    distance = calculateDistance(Xcor5[jj], Ycor5[jj], boxes[jjj][1], boxes[jjj][0])
                                    min_dist1.append(distance)
                            min_dist1.sort()
                            print(min_dist1)
                            if min_dist1[0] > 50:
        ...
        for xx,yy in enumerate(b):
            if yy == 7:
                ...
                for mm,nn in enumerate(class_ids):
                    if nn == 7:
                        distance = calculateDistance(Xcor2[xx], Ycor2[xx], boxes[mm][1], boxes[mm][0])
                        min_dist.append(distance)
                min_dist.sort()
                pseudo_mask = pseudo_b[:,:,xx]
                print('minimum_distance for :', xx, 'is : ', min_dist)
                if min_dist[0] > 50:
                    for jj, kk in enumerate(a):
                        if kk == 7:
                            ...
                            for jjj,kkk in enumerate(class_ids):
                                if kkk == 7:
                                    distance = calculateDistance(Xcor1[jj], Ycor1[jj], boxes[jjj][1], boxes[jjj][0])
                                    min_dist1.append(distance)
                            min_dist1.sort()
                            print(min_dist1)
                            if min_dist1[0] > 50:
        ...
        print('class_ids :', class_ids)
        print('scores_class_ids : ', scores)
        for xx,yy in enumerate(c):
            if yy == 7:
                min_dist = []
                for mm,nn in enumerate(class_ids):
                    if nn == 7:
                        distance = calculateDistance(Xcor3[xx], Ycor3[xx], boxes[mm][1], boxes[mm][0])
                        min_dist.append(distance)
                min_dist.sort()
                pseudo_mask = pseudo_c[:,:,xx]
                print('minimum_distance for :', xx, 'is : ', min_dist)
                if min_dist[0] > 50:
                    for jj, kk in enumerate(b):
                        if kk == 7:
                            min_dist1 = []
                            for jjj,kkk in enumerate(class_ids):
                                if kkk == 7:
                                    distance = calculateDistance(Xcor2[jj], Ycor2[jj], boxes[jjj][1], boxes[jjj][0])
                                    min_dist1.append(distance)
                            min_dist1.sort()
                            print(min_dist1)
                            if min_dist1[0] > 50:
        ...
        print('class_ids :', class_ids)
        print('scores_class_ids : ', scores)
        for xx,yy in enumerate(d):
            if yy == 7:
                min_dist = []
                for mm,nn in enumerate(class_ids):
                    if nn == 7:
                        distance = calculateDistance(Xcor4[xx], Ycor4[xx], boxes[mm][1], boxes[mm][0])
                        min_dist.append(distance)
                    elif 7 not in class_ids:
                        distance = 1000
                        min_dist.append(distance)
                min_dist.sort()
                pseudo_mask = pseudo_d[:,:,xx]
                print('minimum_distance for :', xx, 'is : ', min_dist)
                if min_dist[0] > 50:
                    for jj, kk in enumerate(c):
                        if kk == 7:
                            min_dist1 = []
                            for jjj,kkk in enumerate(class_ids):
                                if kkk == 7:
                                    distance = calculateDistance(Xcor3[jj], Ycor3[jj], boxes[jjj][1], boxes[jjj][0])
                                    min_dist1.append(distance)
                                elif 7 not in class_ids:
                                    distance = 1000
                                    min_dist1.append(distance)
