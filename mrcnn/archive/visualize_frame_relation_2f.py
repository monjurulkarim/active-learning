...
def check(x1,Xcoordinate):
    if Xcoordinate-60 <= x1 <= Xcoordinate+60:
        return True
    return False

def check_y(y1, Ycoordinate):
    if Ycoordinate-60 <= y1 <= Ycoordinate+60:
        return True
    return False


def display_instances(image, boxes, masks, class_ids, class_names,a,Xcor,Ycor,
                      scores=None, title="",
...
# The following block will be used for three frames reference        
#         for x,y in enumerate(scores1):
#             if y == class_ids[i]:
#                 Xcoordinate = Xcor3[x]
#                 print('from visualize: ','for classi_d: ',class_ids[i],'value Xcor :', Xcoordinate)
#                 Ycoordinate = Ycor3[x]
...
def display_instances_back_forth(image, boxes, masks, class_ids, class_names,a2,Xcor2,Ycor2,a1,Xcor1,Ycor1,a,Xcor,Ycor,
                      scores=None, title="",
                      figsize=(16, 16), ax=None,
                      show_mask=True, show_bbox=False, #Raju: I changed "show_bbox = False", previously =True
                      colors=None, captions=None):
...
#                 print('from visualize: ','for classi_d: ',class_ids[i],'value Ycor :', Ycoordinate)
        ...
        if not captions:
            ...
            if score >= 0.9:
                caption = "{} {:.3f}".format(label, score) if score else label
            elif score <0.9 and class_id in a2 and class_id in a and check(x1,Xcoordinate)==True and check_y(y1,Ycoordinate)== True:
                caption = "{} {:.3f}".format(label, score) if score else label
#             elif score <0.9 and score >.50 and class_id in a and check(x1,Xcoordinate)==True and check_y(y1,Ycoordinate)== True:
#                 caption = "{} {:.3f}".format(label, score) if score else label
            else:
...
#                 caption = captions[i]
        ...
        if show_mask:
            ...
            elif score <0.9 and scores0>=0.9:
                mask_color(label,masked_image, mask)
#             elif score <0.9 and class_id in a2 and class_id in a and check(x1,Xcoordinate)==True and check_y(y1,Ycoordinate)== True:
#                 mask_color(label,masked_image, mask)
#             elif score <0.9 and score >.50 and class_id in a and check(x1,Xcoordinate)==True and check_y(y1,Ycoordinate)== True:
